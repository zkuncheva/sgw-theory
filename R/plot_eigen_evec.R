...
l.vec = 1:dim(Vecs)[1]
for (i in 1:6){
  plot(l.vec,Vecs[,i], col=cls.full, type='n', ylab='Eigenvector', xlab='Time layers represented by nodes', main=paste0('Eigenvector ',i)
  ...
  axis(1, at=seq(from=50, to=2950, by=100), labels=c(1:30), las=2, cex.axis=1.5)
  
}

plot(c(-1,0,1),c(0,0,0),xlim=c(-20,20),pch=16,bty ="n",axes=F,frame.plot=F, xaxt='n', ann=FALSE, yaxt='n')

plot(Vecs[,35],col=cls.full,type='n', ylab='Eigenvector', xlab='Time layers represented by nodes', main=paste0('Eigenvector ',35)
