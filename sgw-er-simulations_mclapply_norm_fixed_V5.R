...
# Просто results.dir в началото го hard code full path name. "/Home/bla/results"
# results.dir="./results/"
# Below is for Windows  ONLY - has to be commented
#setwd('D:\\Ogggi\\JANA\\2017_November_Paper_NETWORKS__JANA_OK____MAINNNNNNNN\\2023, CORRECTIONS__PROGRAM__REFEREE\\')
...
sim.func=function(params){
  ...
  Err = NULL
  for (j in 1:100){
    beta = as.numeric(lm(sVec[,j]~0+Mat0)$coefficients)  # 0+Mat0 removes the intercept - which is default in the Matlab mvregress
...
##  This has cores=1 only for Windows; for HPC has say  75, etc.
mclapply(params, sim.func, mc.cores=6) 
